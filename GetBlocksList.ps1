...
Set-Content CheckList "Проверка целостности объектов"
for ($i = $MinX; $i -le $MaxX; $i += 1) {
    Add-Content CheckList ("")
    Add-Content CheckList ("Уровень "+$i)
    Add-Content CheckList ("")
    $CheckList = [string[]]""
    for ($j = $MinY; $j -le $MaxY; $j += 1) {
        ...
        for ($k = $MinZ; $k -le $MaxZ; $k += 1) {
            ...
            if ($Node.Count -eq 0) {
                ...
                $Row += "O"
                $CheckList += ($Node.Node.ParentNode.SubtypeName+" "+$Node.Node.ParentNode.CustomName)
            }
        }
        Add-Content CheckList $Row
    }
    Add-Content CheckList $CheckList
}
